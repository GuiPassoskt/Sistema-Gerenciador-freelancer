          ...
          <div class="row">
			...
			<?php if(($estimate->estimate_status == "Open" || $estimate->estimate_status == "Revised") && isset($estimate->company->name)){ ?><a href="<?=base_url()?>estimates/sendestimate/<?=$estimate->id;?>" class="btn btn-primary"><i class="fa fa-envelope visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_send_estimate_to_client');?></span></a><?php } ?>
			...
			<?php if($estimate->estimate_status == "Invoiced"){ ?><a href="<?=base_url()?>invoices/view/<?=$estimate->id;?>" class="btn btn-success"><i class="fa fa-envelope visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_go_to_invoice');?></span></a><?php } ?>
		...
		<ul class="details col-xs-12 col-sm-6">
			<li><span><?=$this->lang->line('application_estimate_id');?>:</span> <?=$core_settings->estimate_prefix;?><?=$estimate->reference;?></li>
		...
		<div class="row">
		<div class="col-md-12">
		<div class="table-head"><?=$this->lang->line('application_items');?> <?php if($estimate->estimate_status != "Accepted" && $estimate->estimate_status != "Invoiced"){ ?><span class=" pull-right"><a href="<?=base_url()?>estimates/item/<?=$estimate->id;?>" class="btn btn-md btn-primary" data-toggle="mainmodal"><i class="fa fa fa-plus visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_add_item');?></span></a></span> <?php } ?></div>
		...
		<?php if ($discount != 0): ?>
		<tr>
			<td colspan="5" align="right"><?=$this->lang->line('application_discount');?> <?php if(isset($discountpercent)){ echo "(".$estimate->discount.")";}?></td>
