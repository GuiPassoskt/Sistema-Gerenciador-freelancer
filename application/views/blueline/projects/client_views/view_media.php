                        ...
                        <div class="timeline-heading">
                            ...
                            <p><small class="text-muted"><span class="writer"><?=$value->from;?></span> <span class="datetime"><?php  $unix = human_to_unix($value->datetime); echo date($core_settings->date_format.' '.$core_settings->date_time_format, $unix); ?></span></small>
                            		<?php if($value->from == $this->client->firstname." ".$this->client->lastname){ ?>
							         <a href="<?=base_url()?>cprojects/deletemessage/<?=$media->project_id;?>/<?=$media->id;?>/<?=$value->id;?>" rel="" class="btn btn-xs btn-danger pull-right"><i class="fa fa-times"></i></a>
							 		 <?php } ?>
                            </p></h5>
                          </div>
...
<ul id="comments-ul" class="comments">
                     ...
                     <!--  <div class="comment-pic">
                      ...
                      <div class="comment-content">
                          ...
                          <div class="comment-content">
                          <h5><?=$media->name;?> </h5>
                            <p><small class="text-muted"><?=$this->lang->line('application_uploaded_by');?> <?php if(isset($media->user->firstname)){ ?><?=$media->user->firstname;?> <?=$media->user->lastname;?><?php }else{ ?> <?=$media->client->firstname;?> <?=$media->client->lastname;?><?php } ?> <?php $unix = human_to_unix($media->date); echo date($core_settings->date_format.' '.$core_settings->date_time_format, $unix); ?></small></p>
