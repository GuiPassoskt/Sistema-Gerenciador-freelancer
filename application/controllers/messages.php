	...
	function filter($condition = FALSE, $con = FALSE)
	{
		...
		switch ($condition) {
			...
			case 'deleted':
				...
				LEFT JOIN clients ON CONCAT("c",clients.id) = privatemessages.sender
				LEFT JOIN users ON CONCAT("u",users.id) = privatemessages.sender 
				GROUP by privatemessages.id HAVING privatemessages.recipient = "u'.$this->user->id.'" AND privatemessages.status = "deleted" OR privatemessages.deleted = 1 ORDER BY privatemessages.`time` DESC LIMIT '.$limit.$max_value;
